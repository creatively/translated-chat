...
document.querySelectorAll('.icon-copy').forEach(icon => icon.addEventListener('click', copyUrlToClipboard))
...
socket.on('info-message-only-user', data => {
  appendMessage({
    textInfo: 
      ...
      <image class="icon-copy" src="./images/icon-copy.png" onclick="copyUrlToClipboard()" />`
  })
})
...
const populateLanguagesSelectLanguageList = () => {
...
const setLanguagesSelectToTheSystemLanguage = () => {
  document.getElementById('select-language').value = navigator.language.substring(0,2)
}

document.body.onload = () => {
  populateLanguagesSelectLanguageList()
  setLanguagesSelectToTheSystemLanguage()
